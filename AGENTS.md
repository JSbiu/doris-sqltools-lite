...
下面一行把本机个人上下文纳入自动加载（相对路径基准 = 本文件所在目录，即仓库根）：

@.local/memory.md

`.local/` 已在 `.gitignore` 中忽略，只对本机生效；文件缺失时该导入静默跳过，不影响其他协作者。

## 协作与文档分工

- 开始工作时读取本文件及根目录 `.local/memory.md`（已由上方导入自动加载），再按任务范围查阅 README 和相关文件。
